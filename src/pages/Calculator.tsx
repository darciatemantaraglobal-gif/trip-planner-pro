                          ...
                          {tierRows.map(({ tier, cells }) => {
                            ...
                            return (
                              <tr key={tier.label} className={cn(
                                ...
                                {rooms.map((r) => {
                                  const cell = cells.get(r);
                                  const profitIDR = cell ? cell.perPaxIDR - cell.hppPerPaxIDR : null;
                                  return (
                                    <td key={r} style={M} className="px-3 py-2 text-right">
                                      <span className="font-mono font-semibold text-orange-700 block">
                                        {cell ? formatCurrency(cell.perPaxDisplay) : "—"}
                                      </span>
                                      {profitIDR !== null && (
                                        <span className="font-mono text-[9px] md:text-[10px] text-emerald-600 block">
                                          +{new Intl.NumberFormat("id-ID", { style: "currency", currency: "IDR", maximumFractionDigits: 0 }).format(profitIDR)}/pax
                                        </span>
                                      )}
                                    </td>
                                  );
