 ...
 *  Return null kalau fetch gagal (network/auth error), biar caller bisa bedain
 *  "gagal fetch" vs "berhasil tapi 0 preset". Kalau null, caller harus fallback
 *  ke cache localStorage dan TIDAK boleh wipe state yang sudah ada. */
export async function pullPdfLayoutPresets(): Promise<IghLayoutPreset[] | null> {
  if (!isSupabaseConfigured()) return null;
  try {
    const agencyId = requireAgencyId();
    const { data, error } = await supabase!
    ...
    console.warn("pullPdfLayoutPresets error", e);
    return null;
  }
}
