...
export function normalizeIsoDate(raw: string | null | undefined): string | undefined {
...
function normalizeOpenAIParsed(parsed: OpenAIParsed): PassportData {
  ...
  const normalizedDob = normalizeIsoDate(parsed.birthDate);
  if (normalizedDob) out.birthDate = normalizedDob;
  const normalizedExp = normalizeIsoDate(parsed.expiryDate);
  if (normalizedExp) out.expiryDate = normalizedExp;
