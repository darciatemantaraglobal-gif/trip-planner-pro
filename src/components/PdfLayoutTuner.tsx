...
const SECTION_LABELS: { key: IghSection; label: string }[] = [
  ...
  { key: "pricingNote", label: "Catatan Tambahan" },
...
export function PdfLayoutTuner({ config, mode = "private", onChange, onClose }: Props) {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      if (cancelled) return;
      // null = fetch gagal (network/auth) → jangan wipe cache yang sudah ada.
      // [] = sukses tapi 0 preset → update state (preset memang kosong).
      if (list !== null) setCloudPresets(list);
      if (activeConfig) {
    ...
    const off = onPdfPresetsChanged(() => {
      void pullPdfLayoutPresets().then((list) => {
        if (!cancelled && list !== null) setCloudPresets(list);
      });
    });
    return () => {
      cancelled = true;
  ...
  return (
    <div className="w-72 shrink-0 border-l border-[hsl(var(--border))] bg-slate-50/80 backdrop-blur-sm flex flex-col">
      ...
      <div className="flex-1 overflow-y-auto px-3 py-3 space-y-4">
        ...
        {/* CATATAN TAMBAHAN (PRICING NOTE) */}
        <section className="space-y-2 rounded-lg border border-amber-200 bg-amber-50/50 p-2">
          <h4 className="text-[10px] font-bold uppercase tracking-wide text-amber-800">
            Catatan Tambahan (Pricing Note)
          </h4>
          <SliderRow
            label="X Position"
            value={local.pricingNote?.xPx ?? 47}
            min={20} max={700} step={1} unit="px"
            onChange={(v) =>
              setLocal((prev) => ({
                ...prev,
                pricingNote: {
                  ...(prev.pricingNote ?? { xPx: 47, topPx: 657, size: 7.5, align: "left", maxWidthPx: 646 }),
                  xPx: v,
                },
              }))
            }
          ...
          <AlignRow
            ...
            value={local.pricingNote?.align ?? "left"}
            onChange={(v) =>
              setLocal((prev) => ({
                ...prev,
                pricingNote: {
          ...
          <SliderRow
            label="Y Position"
            value={local.pricingNote?.topPx ?? 657}
            min={550} max={780} step={1} unit="px"
            onChange={(v) =>
              setLocal((prev) => ({
                ...prev,
                pricingNote: {
          ...
          <SliderRow
            label="Font Size"
            value={local.pricingNote?.size ?? 7.5}
            min={6} max={16} step={0.5} unit="pt"
            onChange={(v) =>
              setLocal((prev) => ({
                ...prev,
                pricingNote: {
          ...
          <SliderRow
            ...
            value={local.pricingNote?.maxWidthPx ?? 646}
            min={100} max={700} step={5} unit="px"
            onChange={(v) =>
              setLocal((prev) => ({
                ...prev,
                pricingNote: {
          ...
          />
          <p className="text-[9px] text-slate-500 leading-snug">
            Atur tata letak catatan tambahan (misal: "Belum Pembayaran", info DP/pelunasan, dll).
          </p>
        </section>
        ...
        <section className="space-y-2">
          ...
          />
          <SliderRow
            label={`X Kolom Kiri — anchor ${(local.checklist.sudahTermasukAlign ?? "center") === "left" ? "(tepi kiri teks)" : (local.checklist.sudahTermasukAlign ?? "center") === "right" ? "(tepi kanan teks)" : "(tengah teks)"}`}
            value={local.checklist.leftXPx}
            min={20} max={320} step={1} unit="px"
          ...
          />
          <SliderRow
            label={`X Kolom Kanan — anchor ${(local.checklist.belumTermasukAlign ?? "center") === "left" ? "(tepi kiri teks)" : (local.checklist.belumTermasukAlign ?? "center") === "right" ? "(tepi kanan teks)" : "(tengah teks)"}`}
            value={local.checklist.rightXPx}
            min={350} max={720} step={1} unit="px"
