...
function buildElements(
  ...
  // ── Pricing Note (Catatan Tambahan) ──
  {
    const pn = layout.pricingNote ?? { xPx: 47, topPx: 657, size: 7.5, align: "left", maxWidthPx: 646 };
    els.push({
      key: "pricingNote",
      label: "Catatan Tambahan",
      xPx: pn.xPx,
      yPx: textBoxY(pn.topPx, pn.size),
      widthPx: pn.maxWidthPx ?? 646,
      heightPx: textBoxH(pn.size),
      size: pn.size,
    });
  }
...
function applyTranslate(
  ...
  switch (key) {
    ...
    case "checklist":
      ...
      break;
    case "pricingNote": {
      const cur = layout.pricingNote ?? { xPx: 47, topPx: 657, size: 7.5, align: "left", maxWidthPx: 646 };
      next.pricingNote = {
        ...cur,
        xPx: cur.xPx + dxPx,
        topPx: cur.topPx + dyPx,
      };
      break;
    }
    case "whatsapp": {
...
function applyResize(
  ...
  switch (key) {
    ...
    case "pricingNote": {
      const cur = layout.pricingNote ?? { xPx: 47, topPx: 657, size: 7.5, align: "left", maxWidthPx: 646 };
      next.pricingNote = { ...cur, size: clamped };
      break;
    }
    case "checklist":
