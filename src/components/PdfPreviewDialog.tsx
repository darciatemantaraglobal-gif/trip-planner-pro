...
import { Download, Loader2, X, Zap, ZapOff } from "lucide-react";
...
export function PdfPreviewDialog({ open, onOpenChange, data }: Props) {
  ...
  // Re-render preview when opened. In live mode, also debounce on data change.
  ...
  useEffect(() => {
    ...
    // First render: immediate. Subsequent (live mode only): debounce 400ms.
    const delay = initial ? 0 : 400;
  ...
  const header = (
    <div className="px-5 pt-3 pb-2 shrink-0 flex items-start justify-between gap-3" style={{ fontFamily: "'Poppins', sans-serif" }}>
      ...
      <button
        ...
        className={`shrink-0 inline-flex items-center gap-1 h-6 px-2 rounded-md text-[10px] font-bold border transition-colors ${
          live
      ...
      <DialogPrimitive.Root open={open} onOpenChange={onOpenChange} modal={false}>
        <DialogPrimitive.Portal>
          <DialogPrimitive.Content
            ...
            className="fixed bottom-4 right-4 z-50 w-[min(420px,calc(100vw-2rem))] h-[min(640px,calc(100vh-2rem))] rounded-2xl border border-[hsl(var(--border))] shadow-2xl bg-white flex flex-col overflow-hidden data-[state=open]:animate-in data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=open]:fade-in-0"
          >
            {header}
            {previewBody}
            {footer}
